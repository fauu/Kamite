...
public class EasyOCRHFAdapter implements RemoteOCRAdapter {
  private static final Logger LOG = LogManager.getLogger(MethodHandles.lookup().lookupClass());

  private static final int REQUEST_TIMEOUT_S = 8;

  private static final URI API_ENDPOINT =
    URI.create("https://tomofi-easyocr.hf.space/api/predict/");
  private static final String OCR_REQUEST_BODY_TPL =
    "{\"data\": [\"data:%s;base64,%s\", [\"ja\"]]}";
  private static final String OCR_RESPONSE_RELEVANT_START_MARKER = "\"data\":[[";
  private static final String OCR_RESPONSE_RELEVANT_END_MARKER = "]]}]";
  ...
  public Result<String, RemoteOCRRequestError> ocr(BufferedImage img) {
    ...
    var textStartMarkerIdx = res.body().indexOf(OCR_RESPONSE_RELEVANT_START_MARKER);
    if (textStartMarkerIdx == -1) {
      return Result.Err(
        new RemoteOCRRequestError.Other("Response did not contain the expected text start marker")
      );
    }
    var textStartIdx = textStartMarkerIdx + OCR_RESPONSE_RELEVANT_START_MARKER.length();
    var textEndMarkerIdx = res.body().indexOf(OCR_RESPONSE_RELEVANT_END_MARKER, textStartIdx);
    ...
    var relevantExtract = res.body().substring(textStartIdx, textEndMarkerIdx);
    var finalTextBuilder = new StringBuilder();
    var m = OCR_RESPONSE_TEXT_FRAGMENT_RE.matcher(relevantExtract);
    while (m.find()) {
      finalTextBuilder.append(m.group(1));
    }

    return Result.Ok(finalTextBuilder.toString());
  }
}
