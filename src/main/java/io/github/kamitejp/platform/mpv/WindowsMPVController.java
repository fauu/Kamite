...
public final class WindowsMPVController implements MPVController {
  private static final Logger LOG = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

  private static final int CONNECTION_RETRY_INTERVAL_MS = 2000;
  private static final String PIPE_NAME = "\\\\.\\pipe\\kamite-mpvsocket";
  ...
  public WindowsMPVController(Platform platform, Consumer<PlayerStatus> statusUpdateCb) {
  ...
  public void sendCommand(MPVCommand cmd) {
    ...
    var ipcCmd = "{\"command\": ["
      ...
      + "}\n";
    try {
      pipe.write(ipcCmd.getBytes(StandardCharsets.UTF_8));
  ...
  public MPVController.State getState() {
    return state;
  }
  ...
  private class Worker implements Runnable {
    ...
    public void run() {
      ...
      var pipeFuture = CompletableFuture.supplyAsync(new Connecter());
      try {
        pipe = pipeFuture.get();
      ...
      LOG.info("Connected to mpv named pipe at {}", PIPE_NAME);

      var readerFuture = CompletableFuture.runAsync(
        new Reader(pipe, messageCb)
      );
      try {
        readerFuture.get();
      } catch (ExecutionException | InterruptedException e) {
  ...
  private static class Connecter implements Supplier<RandomAccessFile> {
    @Override
    public RandomAccessFile get() {
      try {
        while (true) {
          var pipe = openConnection();
          if (pipe != null) {
            return pipe;
          }
    ...
    private static RandomAccessFile openConnection() {
      try {
        return new RandomAccessFile(PIPE_NAME, "rw");
      } catch (FileNotFoundException e) {
        if (LOG.isTraceEnabled()) {
          LOG.trace(
            "Could not connect to mpv named pipe at {}: {}",
            PIPE_NAME,
            e.getMessage()
          );
        }
  ...
  private static class Reader implements Runnable {
    ...
    Reader(RandomAccessFile pipe, Consumer<String> messageCb) {
      this.pipe = pipe;
      this.messageCb = messageCb;
    }
    ...
    private String read() throws IOException {
      return pipe.readLine(); // XXX: Errors
    }
  }
}
